...
router.post('/downgrade', async (req, res) => {
  try {
    ...
    console.log(`🔄 [PerfectPay] Iniciando downgrade para usuário ${userId}:`, {
      newPlanId,
      reason
    });
    ...
    // Verificar se é realmente um downgrade (preço menor)
    ...
    const webhookPayload = {
      ...
      product: {
        ...
        external_reference: `downgrade_${userId}_${newPlanId}_${timestamp}`
      },
      plan: {
    ...
    if (webhookResult.processed) {
      res.json({
        ...
        note: `Downgrade de ${currentSubscription.payment_plans.display_name} para ${newPlan.display_name}`
      });
    } else {
      res.status(500).json({
        success: false,
        message: webhookResult.error || 'Erro ao processar downgrade'
      });
    }

  } catch (error) {
    console.error('❌ [PerfectPay] Erro no downgrade:', error.message);
    res.status(500).json({
      success: false,
      message: 'Erro interno do servidor',
      error: error.message
