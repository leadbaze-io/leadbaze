...
async function testRenewalEnterprise() {
  console.log('🚀 ===== TESTE: RENOVAÇÃO MENSAL ENTERPRISE =====\n');
  ...
  try {
    ...
    // 2. Simular webhook de renovação (1 mês depois)
    ...
    const webhookPayload = {
      ...
      // Dados do produto/plano (mesmo plano)
      product: {
        name: "LeadBaze Enterprise",
        external_reference: `renewal_${TEST_USER_ID}_3_${timestamp}` // renewal_userId_planId_timestamp
      },
      plan: {
    ...
    if (webhookResponse.data.success && webhookResponse.data.result.processed) {
      ...
      if (updatedResponse.data.success && updatedResponse.data.data) {
        ...
        if (current) {
          ...
          if (leadsAdded === 10000) {
            console.log('✅ Renovação correta! Adicionou exatamente 10000 leads do plano Enterprise');
          } else {
            console.log(`⚠️  Atenção: Esperado 10000 leads, mas adicionou ${leadsAdded}`);
          }
        } else {
